...
| 运行测试 | 执行 `cd python && python -m algorithms.run_all` 验证所有 57 个 Python 模块 |

**Python 实现目录**：`python/algorithms/`
- `data_structures/` — 18 种数据结构（SequentialList、SinglyLinkedList、BST、AVLTree、Trie 等）
- `sorting/` — 10 种排序算法
- `searching/` — 4 种搜索算法
- `string_algorithm/` — 6 种字符串算法
- `dynamic_programming/` — 9 个经典 DP 问题
- `greedy/` — 3 种贪心算法
...
| 学习阶段 | 模块 | 项目源码路径 | 核心知识点 | 前置依赖 |
|---------|------|------------|-----------|---------|
| 阶段一 | C/C++ 基础 | 全项目 `.c` / `.cpp` 文件 | 指针、内存管理、RAII、CMake | 无 |
| 阶段二 | 顺序表 | `src/linear_list/sequential_list/` | 动态数组、扩容策略 | 阶段一 |
| 阶段二 | 简单排序 | `src/sorting/` | 冒泡/选择/插入排序 | 顺序表 |
| 阶段二 | 二分搜索 | `src/searching/` | 二分搜索及变体 | 有序数组 |
| 阶段三 | 链表 | `src/linear_list/` | 单链表/双链表/循环链表/跳表 | 阶段一 |
| 阶段三 | 栈 | `src/stack/` | 顺序栈、链栈、表达式求值 | 链表/顺序表 |
| 阶段三 | 队列 | `src/queue/` | 循环队列、链队列、双端队列、优先队列 | 链表/顺序表 |
| 阶段四 | 二叉树 | `src/tree/binary_tree/` | 遍历（递归+迭代） | 栈、队列 |
| 阶段四 | BST/AVL | `src/tree/` | 搜索树、平衡旋转 | 二叉树 |
| 阶段四 | 哈夫曼/Trie | `src/tree/` | 编码树、前缀树 | 二叉树、优先队列 |
| 阶段四 | 堆与堆排序 | `src/heap/` + `src/sorting/` | 堆操作、堆排序 | 完全二叉树 |
| 阶段四 | 归并/快排 | `src/sorting/` | 分治排序 | 递归 |
| 阶段四 | 非比较排序 | `src/sorting/` | 计数/基数/桶排序 | 顺序表 |
| 阶段五 | 哈希表 | `src/hash_table/` | 链地址法、开放地址法 | 顺序表、链表 |
| 阶段六 | 图 | `src/graph/` | 邻接矩阵/表、BFS/DFS、MST、最短路径 | 树遍历、栈、队列 |
| 阶段六 | 并查集 | `src/graph/union_find/` | 路径压缩、按秩合并 | 图基础 |
| 阶段七 | 字符串算法 | `src/string_algorithm/` | KMP/Rabin-Karp/Boyer-Moore/Manacher/LCS | 阶段一至六 |
| 阶段八 | 分治 | `src/advanced_algorithm/divide_and_conquer/` | 最近点对、最大子数组 | 归并/快排 |
| 阶段八 | 动态规划 | `src/advanced_algorithm/dynamic_programming/` | 背包/LIS/编辑距离 | 分治 |
| 阶段八 | 贪心 | `src/advanced_algorithm/greedy/` | 活动选择/分数背包 | DP |
| 阶段八 | 回溯 | `src/advanced_algorithm/backtracking/` | N皇后/全排列/组合 | DFS |
| 阶段九 | 位运算/数学 | `src/bit_manipulation/`、`src/math/` | 位操作、GCD、快速幂 | C/C++ 基础 |
