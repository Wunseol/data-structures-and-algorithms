...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [01_sequential_list_v1.cpp](../../src/linear_list/sequential_list/01_sequential_list_v1.cpp#L6) | `SqList` (L6), `InsList` (L332), `DelList` (L353) |
> | Python | [L01_sequential_list.py](../../python/algorithms/data_structures/L01_sequential_list.py#L17) | `SequentialList` (L17) |
...
**顺序表内存布局**（对应 `01_sequential_list_v1.cpp` 中 `SqList` 结构体，L6-11）：
...
**插入操作流程**（对应 `InsList` 函数，01_sequential_list_v1.cpp#L332-351）：

```mermaid
flowchart TD
    A["InsList: 开始插入"] --> B{"检查位置合法性<br/>i&lt;1 || i&gt;length+1?<br/><!-- src: 01_sequential_list_v1.cpp#L334 -->"}
    B -->|非法| C["返回 false"]
    B -->|合法| D{"检查容量<br/>size == length?<br/><!-- src: 01_sequential_list_v1.cpp#L335 -->"}
    D -->|已满| E["倍增扩容<br/>new int[2*size]<br/>复制 + 更新size<br/><!-- src: 01_sequential_list_v1.cpp#L336-343 -->"]
    D -->|未满| F["元素后移<br/>for k=length-1 to i-1<br/>data[k+1]=data[k]<br/><!-- src: 01_sequential_list_v1.cpp#L345-347 -->"]
    E --> F
    F --> G["插入元素<br/>data[i-1] = e<br/><!-- src: 01_sequential_list_v1.cpp#L348 -->"]
    G --> H["更新长度<br/>length++<br/><!-- src: 01_sequential_list_v1.cpp#L349 -->"]
    H --> I["返回 true"]
```

#### 核心代码片段

👉 [跳转到完整源码](../../src/linear_list/sequential_list/01_sequential_list_v1.cpp)
...
👉 [插入操作 — InsList](../../src/linear_list/sequential_list/01_sequential_list_v1.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L01_sequential_list.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [04_singly_linked_list_v1.c](../../src/linear_list/singly_linked_list/04_singly_linked_list_v1.c#L7) | `LinkList` (L7), `CreateListH` (L22), `CreateListL` (L37) |
> | Python | [L04_singly_linked_list.py](../../python/algorithms/data_structures/L04_singly_linked_list.py#L17) | `SinglyLinkedList` (L17) |
...
**单链表节点结构**（对应 `04_singly_linked_list_v1.c` 中 `LinkList` 结构体，L7-10）：
...
**头插法建表流程**（对应 `CreateListH` 函数，04_singly_linked_list_v1.c#L22-34）：

```mermaid
flowchart TD
    A["CreateListH: 开始"] --> B["malloc 新节点 s<br/><!-- src: 04_singly_linked_list_v1.c#L28 -->"]
    B --> C["读入数据 s->data<br/><!-- src: 04_singly_linked_list_v1.c#L29 -->"]
    C --> D["s->next = head->next<br/><!-- src: 04_singly_linked_list_v1.c#L30 -->"]
    D --> E["head->next = s<br/><!-- src: 04_singly_linked_list_v1.c#L31 -->"]
    E --> F{"还有更多元素?"}
    F -->|是| B
    F -->|否| G["建表完成<br/>结果为逆序"]
```

**单链表节点链接结构**（对应 `04_singly_linked_list_v1.c` 中 `LinkList` 结构体）：
...
> 结构体定义（`04_singly_linked_list_v1.c`）：
> ```c
...
👉 [跳转到完整源码](../../src/linear_list/singly_linked_list/04_singly_linked_list_v1.c)
...
👉 [尾插法建表](../../src/linear_list/singly_linked_list/04_singly_linked_list_v1.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L04_singly_linked_list.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [doubly_linked_list.h](../../src/linear_list/doubly_linked_list/doubly_linked_list.h#L9) / [.c](../../src/linear_list/doubly_linked_list/05_doubly_linked_list.c#L52) | `DuLNode` (L9), `ListInsert` (L52), `ListDelete` (L83) |
> | Python | [L05_doubly_linked_list.py](../../python/algorithms/data_structures/L05_doubly_linked_list.py#L17) | `DoublyLinkedList` (L17) |
...
**插入操作流程**（对应 `ListInsert` 函数，05_doubly_linked_list.c#L52-81）：

```mermaid
flowchart TD
    A["ListInsert: 开始"] --> B{"检查位置合法性<br/>i &lt; 1?<br/><!-- src: 05_doubly_linked_list.c#L54 -->"}
    B -->|非法| C["返回 0"]
    B -->|合法| D["遍历找前驱节点 p<br/><!-- src: 05_doubly_linked_list.c#L58-63 -->"]
    D --> E{"p == NULL?<br/><!-- src: 05_doubly_linked_list.c#L64 -->"}
    E -->|是| F["位置超出范围<br/>返回 0"]
    E -->|否| G["malloc 新节点 s<br/><!-- src: 05_doubly_linked_list.c#L68 -->"]
    G --> H["s->data = e<br/><!-- src: 05_doubly_linked_list.c#L73 -->"]
    H --> I["s->next = p->next<br/><!-- src: 05_doubly_linked_list.c#L74 -->"]
    I --> J["s->prior = p<br/><!-- src: 05_doubly_linked_list.c#L75 -->"]
    J --> K{"p->next != NULL?<br/><!-- src: 05_doubly_linked_list.c#L76 -->"}
    K -->|是| L["p->next->prior = s<br/><!-- src: 05_doubly_linked_list.c#L77 -->"]
    K -->|否| M["p->next = s<br/><!-- src: 05_doubly_linked_list.c#L79 -->"]
    L --> M
    M --> N["返回 1"]
```
...
👉 [插入操作 — ListInsert](../../src/linear_list/doubly_linked_list/05_doubly_linked_list.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L05_doubly_linked_list.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [circular_linked_list.h](../../src/linear_list/circular_linked_list/circular_linked_list.h#L9) / [.c](../../src/linear_list/circular_linked_list/06_circular_linked_list.c#L14) | `CiLNode` (L9), `CreateCiListTail` (L14), `MergeCiList` (L114) |
> | Python | [L06_circular_linked_list.py](../../python/algorithms/data_structures/L06_circular_linked_list.py#L16) | `CircularLinkedList` (L16) |
...
**循环链表环形结构**（对应 `circular_linked_list.h` 中 `CiLNode` 结构体，L9-12；初始化 `L->next = L`，06_circular_linked_list.c#L10）：
...
**归并合并流程**（对应 `MergeCiList` 函数，06_circular_linked_list.c#L114-147）：

```mermaid
flowchart TD
    A["MergeCiList: 开始"] --> B["初始化指针<br/>pa=La->next, pb=Lb->next, pc=Lc<br/><!-- src: 06_circular_linked_list.c#L116-118 -->"]
    B --> C{"pa != La && pb != Lb?<br/><!-- src: 06_circular_linked_list.c#L120 -->"}
    C -->|是| D{"pa->data <= pb->data?<br/><!-- src: 06_circular_linked_list.c#L121 -->"}
    D -->|是| E["pc->next = pa<br/>pc = pa, pa = pa->next<br/><!-- src: 06_circular_linked_list.c#L122-124 -->"]
    D -->|否| F["pc->next = pb<br/>pc = pb, pb = pb->next<br/><!-- src: 06_circular_linked_list.c#L126-128 -->"]
    E --> C
    F --> C
    C -->|否| G{"pa != La?<br/><!-- src: 06_circular_linked_list.c#L132 -->"}
    G -->|是| H["链接La剩余节点<br/><!-- src: 06_circular_linked_list.c#L132-136 -->"]
    G -->|否| I{"pb != Lb?<br/><!-- src: 06_circular_linked_list.c#L137 -->"}
    I -->|是| J["链接Lb剩余节点<br/><!-- src: 06_circular_linked_list.c#L137-141 -->"]
    H --> K["闭合循环: pc->next = Lc<br/><!-- src: 06_circular_linked_list.c#L143 -->"]
    J --> K
    I -->|否| K
    K --> L["重置源链表<br/>La->next=La, Lb->next=Lb<br/><!-- src: 06_circular_linked_list.c#L145-146 -->"]
```
...
👉 [归并合并 — MergeCiList](../../src/linear_list/circular_linked_list/06_circular_linked_list.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L06_circular_linked_list.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [skip_list.h](../../src/linear_list/skip_list/skip_list.h#L6) / [.cpp](../../src/linear_list/skip_list/09_skip_list.cpp#L6) | `SkipNode` (L6), `SkipList` (L12), `Insert` (L42), `Search` (L76) |
> | Python | [L09_skip_list.py](../../python/algorithms/data_structures/L09_skip_list.py#L16) | `SkipList` (L16) |
...
**查找操作流程**（对应 `Search` 函数，09_skip_list.cpp#L76-90）：

```mermaid
flowchart TD
    A["Search: 开始查找"] --> B["cur = header_<br/>从最高层开始<br/><!-- src: 09_skip_list.cpp#L78 -->"]
    B --> C{"逐层向下<br/>i = level-1 to 0<br/><!-- src: 09_skip_list.cpp#L79 -->"}
    C --> D{"cur->forward[i]->key &lt; key?<br/><!-- src: 09_skip_list.cpp#L80 -->"}
    D -->|是| E["cur = cur->forward[i]<br/>向右移动<br/><!-- src: 09_skip_list.cpp#L81 -->"]
    E --> D
    D -->|否| F["下降一层<br/><!-- src: 09_skip_list.cpp#L79 -->"]
    F --> C
    C -->|遍历完所有层| G["cur = cur->forward[0]<br/><!-- src: 09_skip_list.cpp#L84 -->"]
    G --> H{"cur != NULL &&<br/>cur->key == key?<br/><!-- src: 09_skip_list.cpp#L85 -->"}
    H -->|是| I["找到: value = cur->value<br/>返回 true<br/><!-- src: 09_skip_list.cpp#L86-87 -->"]
    H -->|否| J["未找到<br/>返回 false<br/><!-- src: 09_skip_list.cpp#L89 -->"]
```
...
👉 [插入操作 — Insert](../../src/linear_list/skip_list/09_skip_list.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L09_skip_list.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [07_sequential_stack_v1.cpp](../../src/stack/sequential_stack/07_sequential_stack_v1.cpp#L12) | `Sqstack` (L12), `Push` (L37), `Pop` (L50) |
> | Python | [L07_stack.py](../../python/algorithms/data_structures/L07_stack.py#L16) | `Stack` (L16) |
...
**顺序栈内存布局**（对应 `07_sequential_stack_v1.cpp` 中 `Sqstack` 结构体，L12-16）：
...
**入栈/出栈流程**（对应 `Push` 函数，07_sequential_stack_v1.cpp#L37-48；`Pop` 函数，07_sequential_stack_v1.cpp#L50-57）：

```mermaid
flowchart TD
    subgraph Push入栈
        P1["Push: 开始"] --> P2{"栈满?<br/>top-base >= stacksize<br/><!-- src: 07_sequential_stack_v1.cpp#L39 -->"}
        P2 -->|是| P3["realloc 扩容<br/>更新top和stacksize<br/><!-- src: 07_sequential_stack_v1.cpp#L40-44 -->"]
        P2 -->|否| P4["*top = e<br/><!-- src: 07_sequential_stack_v1.cpp#L45 -->"]
        P3 --> P4
        P4 --> P5["top++<br/><!-- src: 07_sequential_stack_v1.cpp#L46 -->"]
    end

    subgraph Pop出栈
        Q1["Pop: 开始"] --> Q2{"栈空?<br/>base == top<br/><!-- src: 07_sequential_stack_v1.cpp#L52 -->"}
        Q2 -->|是| Q3["返回 ERROR"]
        Q2 -->|否| Q4["e = *(top-1)<br/><!-- src: 07_sequential_stack_v1.cpp#L54 -->"]
        Q4 --> Q5["top--<br/><!-- src: 07_sequential_stack_v1.cpp#L55 -->"]
    end
```

#### 核心代码片段

👉 [跳转到完整源码](../../src/stack/sequential_stack/07_sequential_stack_v1.cpp)
...
👉 [入栈与出栈操作](../../src/stack/sequential_stack/07_sequential_stack_v1.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L07_stack.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [07_linked_stack_v1.cpp](../../src/stack/linked_stack/07_linked_stack_v1.cpp#L15) | `StackNode` (L15), `Push` (L39), `Pop` (L52) |
> | Python | [L07_stack.py](../../python/algorithms/data_structures/L07_stack.py#L16) | `Stack` (L16) |
...
👉 [跳转到完整源码](../../src/stack/linked_stack/07_linked_stack_v1.cpp)
...
👉 [入栈与出栈操作](../../src/stack/linked_stack/07_linked_stack_v1.cpp)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [circular_queue.h](../../src/queue/circular_queue/circular_queue.h#L10) / [.c](../../src/queue/circular_queue/08_circular_queue.c#L4) | `SqQueue` (L10), `EnQueue` (L25), `DeQueue` (L34) |
> | Python | [L08_queue.py](../../python/algorithms/data_structures/L08_queue.py#L17) | `Queue` (L17) |
...
**入队/出队流程**（对应 `EnQueue` 函数，08_circular_queue.c#L25-32；`DeQueue` 函数，08_circular_queue.c#L34-41）：

```mermaid
flowchart TD
    subgraph EnQueue入队
        E1["EnQueue: 开始"] --> E2{"队满?<br/>(rear+1)%MAXSIZE == front<br/><!-- src: 08_circular_queue.c#L27 -->"}
        E2 -->|是| E3["返回 false"]
        E2 -->|否| E4["data[rear] = e<br/><!-- src: 08_circular_queue.c#L29 -->"]
        E4 --> E5["rear = (rear+1) % MAXSIZE<br/><!-- src: 08_circular_queue.c#L30 -->"]
        E5 --> E6["返回 true"]
    end

    subgraph DeQueue出队
        D1["DeQueue: 开始"] --> D2{"队空?<br/>front == rear<br/><!-- src: 08_circular_queue.c#L36 -->"}
        D2 -->|是| D3["返回 false"]
        D2 -->|否| D4["*e = data[front]<br/><!-- src: 08_circular_queue.c#L38 -->"]
        D4 --> D5["front = (front+1) % MAXSIZE<br/><!-- src: 08_circular_queue.c#L39 -->"]
        D5 --> D6["返回 true"]
    end
```
...
👉 [入队与出队操作](../../src/queue/circular_queue/08_circular_queue.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L08_queue.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [linked_queue.h](../../src/queue/linked_queue/linked_queue.h#L8) / [.c](../../src/queue/linked_queue/08_linked_queue.c#L5) | `QNode` (L8), `LinkQueue` (L13), `EnQueue` (L29), `DeQueue` (L41) |
> | Python | [L08_queue.py](../../python/algorithms/data_structures/L08_queue.py#L17) | `Queue` (L17) |
...
👉 [入队与出队操作](../../src/queue/linked_queue/08_linked_queue.c)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [deque.h](../../src/queue/deque/deque.h#L8) / [.cpp](../../src/queue/deque/08_deque.cpp#L11) | `Deque` (L8), `PushFront` (L11), `PushBack` (L21) |
> | Python | [L08_queue.py](../../python/algorithms/data_structures/L08_queue.py#L17) | `Queue` (L17) |
...
👉 [前端入队与后端入队](../../src/queue/deque/08_deque.cpp)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [priority_queue.h](../../src/queue/priority_queue/priority_queue.h#L8) / [.c](../../src/queue/priority_queue/08_priority_queue.c#L12) | `PriorityQueue` (L8), `sift_up` (L12), `sift_down` (L25) |
> | Python | [L15_max_heap.py](../../python/algorithms/data_structures/L15_max_heap.py#L16) | `MaxHeap` (L16) |
...
👉 [上浮与下沉操作](../../src/queue/priority_queue/08_priority_queue.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L15_max_heap.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [10_binary_tree_v1.cpp](../../src/tree/binary_tree/10_binary_tree_v1.cpp#L3) / [bint.h](../../src/tree/binary_tree/bint.h#L171) | `BiTNode` (L3), `PreOrde` (L171), `InOrder` (L185) |
> | Python | [L10_binary_tree.py](../../python/algorithms/data_structures/L10_binary_tree.py#L19) | `BinaryTree` (L19) |
...
👉 [跳转到完整源码](../../src/tree/binary_tree/10_binary_tree_v1.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L10_binary_tree.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [11_bst_crud.cpp](../../src/tree/binary_search_tree/11_bst_crud.cpp#L8) / [bint.h](../../src/tree/binary_tree/bint.h#L392) | `BstNode` (L8), `SearchBstNode` (L188), `InsertBST` (L392) |
> | Python | [L11_binary_search_tree.py](../../python/algorithms/data_structures/L11_binary_search_tree.py#L16) | `BST` (L16) |
...
本项目提供了两种 BST 实现方式。`11_bst_crud.cpp` 采用迭代方式插入结点：从根结点出发，比当前结点小则走左子树，大则走右子树，直到找到空位插入。查找操作同样从根开始逐层比较。`bint.h` 中则提供了递归方式的插入和查找实现，以及完整的删除操作（处理叶子结点、单子树和双子树三种情况）。

#### 结构与流程图

**BST 查找流程**（对应 `SearchBstNode` 函数，11_bst_crud.cpp#L188-208）：

```mermaid
flowchart TD
    A["SearchBstNode: 开始查找"] --> B{"T == NULL?<br/><!-- src: 11_bst_crud.cpp#L191 -->"}
    B -->|是| C["返回 NULL"]
    B -->|否| D{"T->data == key?<br/><!-- src: 11_bst_crud.cpp#L195 -->"}
    D -->|是| E["返回 T"]
    D -->|否| F{"key &lt; T->data?<br/><!-- src: 11_bst_crud.cpp#L199 -->"}
    F -->|是| G["递归查找左子树<br/>SearchBstNode(T->Left, key)<br/><!-- src: 11_bst_crud.cpp#L201 -->"]
    F -->|否| H["递归查找右子树<br/>SearchBstNode(T->Right, key)<br/><!-- src: 11_bst_crud.cpp#L205 -->"]
```
...
**BST 删除流程**（对应 `DeleteBstNode` 函数，11_bst_crud.cpp#L90-166）：

```mermaid
flowchart TD
    A["DeleteBstNode: 开始删除"] --> B["查找父节点<br/>FindFather<br/><!-- src: 11_bst_crud.cpp#L98 -->"]
    B --> C{"找到待删节点?"}
    C -->|否| D["返回原树"]
    C -->|是| E{"无左子树?<br/><!-- src: 11_bst_crud.cpp#L125 -->"}
    E -->|是| F["用右子节点替换<br/><!-- src: 11_bst_crud.cpp#L128 -->"]
    E -->|否| G{"无右子树?<br/><!-- src: 11_bst_crud.cpp#L135 -->"}
    G -->|是| H["用左子节点替换<br/><!-- src: 11_bst_crud.cpp#L138 -->"]
    G -->|否| I["找左子树最右节点<br/>(中序前驱)替换数据<br/><!-- src: 11_bst_crud.cpp#L145-152 -->"]
```

#### 核心代码片段

👉 [跳转到完整源码](../../src/tree/binary_search_tree/11_bst_crud.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L11_binary_search_tree.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [avl_tree.h](../../src/tree/avl_tree/avl_tree.h#L4) / [.cpp](../../src/tree/avl_tree/12_avl_tree.cpp#L37) | `AVLNode` (L4), `AVLTree` (L11), `RotateLL` (L37), `Insert` (L69) |
> | Python | [L12_avl_tree.py](../../python/algorithms/data_structures/L12_avl_tree.py#L16) | `AVLTree` (L16) |
...
**LL 型旋转（右旋）**（对应 `RotateLL`，12_avl_tree.cpp#L37-45）：
```mermaid
graph TD
    subgraph 旋转前
        A1["A (bf=2)"] --> B1["B (bf=1)"]
...
**RR 型旋转（左旋）**（对应 `RotateRR`，12_avl_tree.cpp#L47-55）：
```mermaid
graph TD
    subgraph 旋转前
        A1["A (bf=-2)"] --> AL1["AL"]
...
**LR 型旋转（先左旋后右旋）**（对应 `RotateLR`，12_avl_tree.cpp#L57-61）：
```mermaid
graph TD
    subgraph 旋转前
        A1["A (bf=2)"] --> B1["B (bf=-1)"]
...
**RL 型旋转（先右旋后左旋）**（对应 `RotateRL`，12_avl_tree.cpp#L63-67）：
```mermaid
graph TD
    subgraph 旋转前
        A1["A (bf=-2)"] --> AL1["AL"]
...
**插入与平衡检查流程**（对应 `Insert` 函数，12_avl_tree.cpp#L69-95）：

```mermaid
flowchart TD
    A["Insert: 开始插入"] --> B{"node == NULL?<br/><!-- src: 12_avl_tree.cpp#L71 -->"}
    B -->|是| C["创建新节点<br/><!-- src: 12_avl_tree.cpp#L72 -->"]
    B -->|否| D{"key &lt; node->key?<br/><!-- src: 12_avl_tree.cpp#L75 -->"}
    D -->|是| E["递归插入左子树<br/><!-- src: 12_avl_tree.cpp#L76 -->"]
    D -->|否| F{"key &gt; node->key?<br/><!-- src: 12_avl_tree.cpp#L77 -->"}
    F -->|是| G["递归插入右子树<br/><!-- src: 12_avl_tree.cpp#L78 -->"]
    F -->|否| H["key已存在,返回<br/><!-- src: 12_avl_tree.cpp#L80 -->"]
    E --> I["UpdateHeight(node)<br/><!-- src: 12_avl_tree.cpp#L82 -->"]
    G --> I
    I --> J["计算平衡因子 bf<br/><!-- src: 12_avl_tree.cpp#L84 -->"]
    J --> K{"bf &gt; 1 && key &lt; left->key?<br/>LL型<br/><!-- src: 12_avl_tree.cpp#L85 -->"}
    K -->|是| L["RotateLL<br/><!-- src: 12_avl_tree.cpp#L86 -->"]
    K -->|否| M{"bf &lt; -1 && key &gt; right->key?<br/>RR型<br/><!-- src: 12_avl_tree.cpp#L87 -->"}
    M -->|是| N["RotateRR<br/><!-- src: 12_avl_tree.cpp#L88 -->"]
    M -->|否| O{"bf &gt; 1 && key &gt; left->key?<br/>LR型<br/><!-- src: 12_avl_tree.cpp#L89 -->"}
    O -->|是| P["RotateLR<br/><!-- src: 12_avl_tree.cpp#L90 -->"]
    O -->|否| Q{"bf &lt; -1 && key &lt; right->key?<br/>RL型<br/><!-- src: 12_avl_tree.cpp#L91 -->"}
    Q -->|是| R["RotateRL<br/><!-- src: 12_avl_tree.cpp#L92 -->"]
    Q -->|否| S["返回 node<br/><!-- src: 12_avl_tree.cpp#L94 -->"]
```
...
👉 [旋转与插入操作](../../src/tree/avl_tree/12_avl_tree.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L12_avl_tree.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [huffman_tree.h](../../src/tree/huffman_tree/huffman_tree.h#L6) / [.cpp](../../src/tree/huffman_tree/13_huffman_tree.cpp#L31) | `HuffmanNode` (L6), `HuffmanTree` (L14), `BuildHuffmanTree` (L31) |
> | Python | 暂未实现 | — |
...
👉 [构建哈夫曼树与生成编码](../../src/tree/huffman_tree/13_huffman_tree.cpp)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [trie.h](../../src/tree/trie/trie.h#L6) / [.cpp](../../src/tree/trie/14_trie.cpp#L28) | `TrieNode` (L6), `Trie` (L14), `Insert` (L28), `Search` (L43) |
> | Python | [L14_trie.py](../../python/algorithms/data_structures/L14_trie.py#L14) | `Trie` (L14) |
...
**插入操作流程**（对应 `Insert` 函数，14_trie.cpp#L28-41）：

```mermaid
flowchart TD
    A["Insert: 开始插入"] --> B["cur = root_<br/><!-- src: 14_trie.cpp#L30 -->"]
    B --> C{"遍历word每个字符<br/><!-- src: 14_trie.cpp#L31 -->"}
    C --> D["idx = c - 'a'<br/><!-- src: 14_trie.cpp#L32 -->"]
    D --> E{"children[idx] == NULL?<br/><!-- src: 14_trie.cpp#L35 -->"}
    E -->|是| F["创建新节点<br/>children[idx] = new TrieNode()<br/><!-- src: 14_trie.cpp#L36 -->"]
    E -->|否| G["cur = cur->children[idx]<br/><!-- src: 14_trie.cpp#L37 -->"]
    F --> G
    G --> C
    C -->|遍历完成| H["cur->is_end = true<br/><!-- src: 14_trie.cpp#L39 -->"]
    H --> I["cur->word_count++<br/><!-- src: 14_trie.cpp#L40 -->"]
```

**查找操作流程**（对应 `Search` 函数，14_trie.cpp#L43-55）：

```mermaid
flowchart TD
    A["Search: 开始查找"] --> B["cur = root_<br/><!-- src: 14_trie.cpp#L45 -->"]
    B --> C{"遍历word每个字符<br/><!-- src: 14_trie.cpp#L46 -->"}
    C --> D["idx = c - 'a'<br/><!-- src: 14_trie.cpp#L47 -->"]
    D --> E{"children[idx] == NULL?<br/><!-- src: 14_trie.cpp#L50 -->"}
    E -->|是| F["返回 false<br/>字符路径不存在"]
    E -->|否| G["cur = cur->children[idx]<br/><!-- src: 14_trie.cpp#L52 -->"]
    G --> C
    C -->|遍历完成| H{"cur->is_end == true?<br/><!-- src: 14_trie.cpp#L54 -->"}
    H -->|是| I["返回 true"]
...
👉 [插入与查找操作](../../src/tree/trie/14_trie.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L14_trie.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [segment_tree.h](../../src/tree/segment_tree/segment_tree.h#L6) / [.cpp](../../src/tree/segment_tree/16_segment_tree.cpp#L3) | `SegmentTree` (L6), `Update` (L10), `Query` (L11), `Build` (L17) |
> | Python | [L16_segment_tree.py](../../python/algorithms/data_structures/L16_segment_tree.py#L15) | `SegmentTree` (L15) |
...
**Build 建树流程**（对应 `Build` 函数，16_segment_tree.cpp#L11-21）：

```mermaid
flowchart TD
    A["Build: 开始建树"] --> B{"left == right?"}
    B -->|是| C["tree[node] = data[left]<br/>叶子节点<br/><!-- src: 16_segment_tree.cpp#L13-14 -->"]
    B -->|否| D["mid = (left+right)/2<br/><!-- src: 16_segment_tree.cpp#L17 -->"]
    D --> E["Build(2*node, left, mid)<br/><!-- src: 16_segment_tree.cpp#L18 -->"]
    E --> F["Build(2*node+1, mid+1, right)<br/><!-- src: 16_segment_tree.cpp#L19 -->"]
    F --> G["tree[node] = tree[2*node] + tree[2*node+1]<br/><!-- src: 16_segment_tree.cpp#L20 -->"]
```

**Query 区间查询流程**（对应 `QueryHelper` 函数，16_segment_tree.cpp#L51-60）：
...
👉 [Build 与 Query 操作](../../src/tree/segment_tree/16_segment_tree.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L16_segment_tree.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [fenwick_tree.h](../../src/tree/fenwick_tree/fenwick_tree.h#L6) / [.cpp](../../src/tree/fenwick_tree/17_fenwick_tree.cpp#L3) | `FenwickTree` (L6), `Update` (L11), `Query` (L12), `Lowbit` (L19) |
> | Python | [L17_fenwick_tree.py](../../python/algorithms/data_structures/L17_fenwick_tree.py#L15) | `FenwickTree` (L15) |
...
**Update 操作流程**（对应 `Update` 函数，17_fenwick_tree.cpp#L16-22）：

```mermaid
flowchart TD
    A["Update: 开始更新"] --> B["index = index<br/><!-- src: 17_fenwick_tree.cpp#L18 -->"]
    B --> C{"index <= n_?"}
    C -->|是| D["tree_[index] += delta<br/><!-- src: 17_fenwick_tree.cpp#L19 -->"]
    D --> E["index += Lowbit(index)<br/><!-- src: 17_fenwick_tree.cpp#L20 -->"]
    E --> C
    C -->|否| F["更新完成"]
```

**Query 操作流程**（对应 `Query` 函数，17_fenwick_tree.cpp#L24-32）：

```mermaid
flowchart TD
    A["Query: 查询前缀和[1,index]"] --> B["sum = 0<br/><!-- src: 17_fenwick_tree.cpp#L26 -->"]
    B --> C{"index > 0?"}
    C -->|是| D["sum += tree_[index]<br/><!-- src: 17_fenwick_tree.cpp#L28 -->"]
    D --> E["index -= Lowbit(index)<br/><!-- src: 17_fenwick_tree.cpp#L29 -->"]
    E --> C
    C -->|否| F["返回 sum"]
```
...
👉 [Update 与 Query 操作](../../src/tree/fenwick_tree/17_fenwick_tree.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L17_fenwick_tree.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [max_heap.h](../../src/heap/max_heap/max_heap.h#L10) / [.c](../../src/heap/max_heap/15_max_heap.c#L11) | `MaxHeap` (L10), `SiftUp` (L11), `SiftDown` (L19), `BuildMaxHeap` (L37) |
> | Python | [L15_max_heap.py](../../python/algorithms/data_structures/L15_max_heap.py#L16) | `MaxHeap` (L16) |
...
**建堆流程**（对应 `BuildMaxHeap` 函数，15_max_heap.c#L37-46）：

```mermaid
flowchart TD
    A["BuildMaxHeap: 开始"] --> B["复制数据到data[1..n]<br/><!-- src: 15_max_heap.c#L42-43 -->"]
    B --> C["从最后一个非叶子节点开始<br/>i = n/2<br/><!-- src: 15_max_heap.c#L44 -->"]
    C --> D["SiftDown(H, i)<br/><!-- src: 15_max_heap.c#L45 -->"]
    D --> E{"i >= 1?"}
    E -->|是| F["i--"]
    F --> D
...
**插入与提取流程**（对应 `MaxHeapInsert` 函数，15_max_heap.c#L48-56；`ExtractMax` 函数，15_max_heap.c#L58-67）：

```mermaid
flowchart TD
    subgraph 插入MaxHeapInsert
        I1["开始插入"] --> I2{"size >= MAXSIZE?<br/><!-- src: 15_max_heap.c#L50 -->"}
        I2 -->|是| I3["返回 false"]
        I2 -->|否| I4["size++<br/>data[size] = e<br/><!-- src: 15_max_heap.c#L52-53 -->"]
        I4 --> I5["SiftUp(H, size)<br/><!-- src: 15_max_heap.c#L54 -->"]
    end

    subgraph 提取ExtractMax
        X1["开始提取"] --> X2{"size == 0?<br/><!-- src: 15_max_heap.c#L60 -->"}
        X2 -->|是| X3["返回 false"]
        X2 -->|否| X4["e = data[1]<br/>data[1] = data[size]<br/>size--<br/><!-- src: 15_max_heap.c#L62-64 -->"]
        X4 --> X5["SiftDown(H, 1)<br/><!-- src: 15_max_heap.c#L65 -->"]
    end
```
...
👉 [上浮与下沉操作](../../src/heap/max_heap/15_max_heap.c)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [min_heap.h](../../src/heap/min_heap/min_heap.h#L10) / [.c](../../src/heap/min_heap/15_min_heap.c#L11) | `MinHeap` (L10), `SiftUp` (L11), `SiftDown` (L19), `BuildMinHeap` (L37) |
> | Python | [L15_min_heap.py](../../python/algorithms/data_structures/L15_min_heap.py#L16) | `MinHeap` (L16) |
...
👉 [上浮与下沉操作](../../src/heap/min_heap/15_min_heap.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L15_min_heap.py)
...
> | Python | [L18_hash_table_chaining.py](../../python/algorithms/data_structures/L18_hash_table_chaining.py#L15) | `HashTableChaining` (L15) |
...
**链地址法插入流程**（对应 `htc_insert` 函数，18_hash_table_chaining.c#L20-35）：

```mermaid
flowchart TD
    A["htc_insert: 开始插入"] --> B["计算哈希值<br/>idx = key % size<br/><!-- src: 18_hash_table_chaining.c#L22 -->"]
    B --> C["遍历桶链表查重<br/><!-- src: 18_hash_table_chaining.c#L23-27 -->"]
    C --> D{"key已存在?"}
    D -->|是| E["返回 0<br/><!-- src: 18_hash_table_chaining.c#L25 -->"]
    D -->|否| F["malloc 新节点<br/><!-- src: 18_hash_table_chaining.c#L28 -->"]
    F --> G["node->key = key<br/>node->next = buckets[idx]<br/><!-- src: 18_hash_table_chaining.c#L30-31 -->"]
    G --> H["buckets[idx] = node<br/>头插法插入<br/><!-- src: 18_hash_table_chaining.c#L32 -->"]
    H --> I["count++<br/><!-- src: 18_hash_table_chaining.c#L33 -->"]
```
...
👉 [Python 实现](../../python/algorithms/data_structures/L18_hash_table_chaining.py)
...
> | Python | [L18_hash_table_open_addressing.py](../../python/algorithms/data_structures/L18_hash_table_open_addressing.py#L15) | `HashTableOpenAddressing` (L15) |
...
**开放地址法插入流程**（对应 `htoa_insert` 函数，18_hash_table_open_addressing.c#L23-48）：

```mermaid
flowchart TD
    A["htoa_insert: 开始插入"] --> B["计算哈希值 h = key % size<br/><!-- src: 18_hash_table_open_addressing.c#L25 -->"]
    B --> C{"线性探测<br/>idx = (h+i) % size<br/><!-- src: 18_hash_table_open_addressing.c#L27-28 -->"}
    C --> D{"slots[idx] == OCCUPIED?<br/><!-- src: 18_hash_table_open_addressing.c#L29 -->"}
    D -->|是| E{"key相同?<br/><!-- src: 18_hash_table_open_addressing.c#L30 -->"}
    E -->|是| F["返回 0<br/>key已存在"]
    E -->|否| C
    D -->|DELETED| G["记录首个DELETED位置<br/><!-- src: 18_hash_table_open_addressing.c#L32 -->"]
    G --> C
    D -->|EMPTY| H["在EMPTY或首个DELETED处插入<br/>slots[idx].key = key<br/>slots[idx].status = OCCUPIED<br/><!-- src: 18_hash_table_open_addressing.c#L34-37 -->"]
    H --> I["count++<br/><!-- src: 18_hash_table_open_addressing.c#L38 -->"]
```
...
👉 [Python 实现](../../python/algorithms/data_structures/L18_hash_table_open_addressing.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [bloom_filter.h](../../src/hash_table/bloom_filter/bloom_filter.h#L7) / [.cpp](../../src/hash_table/bloom_filter/19_bloom_filter.cpp#L4) | `BloomFilter` (L7), `Add` (L15), `Contains` (L22) |
> | Python | [L19_bloom_filter.py](../../python/algorithms/data_structures/L19_bloom_filter.py#L16) | `BloomFilter` (L16) |
...
**Add 操作流程**（对应 `Add` 函数，19_bloom_filter.cpp#L15-20）：

```mermaid
flowchart TD
    A["Add: 开始添加"] --> B["i = 0<br/><!-- src: 19_bloom_filter.cpp#L17 -->"]
    B --> C{"i < num_hashes_?"}
    C -->|是| D["idx = HashWithSeed(item, i)<br/><!-- src: 19_bloom_filter.cpp#L18 -->"]
    D --> E["bits_[idx] = true<br/><!-- src: 19_bloom_filter.cpp#L18 -->"]
    E --> F["i++"]
    F --> C
    C -->|否| G["添加完成"]
```

**Contains 操作流程**（对应 `Contains` 函数，19_bloom_filter.cpp#L22-29）：

```mermaid
flowchart TD
    A["Contains: 开始查询"] --> B["i = 0<br/><!-- src: 19_bloom_filter.cpp#L24 -->"]
    B --> C{"i < num_hashes_?"}
    C -->|是| D["idx = HashWithSeed(item, i)<br/><!-- src: 19_bloom_filter.cpp#L25 -->"]
    D --> E{"bits_[idx] == false?"}
...
👉 [Add 与 Contains 操作](../../src/hash_table/bloom_filter/19_bloom_filter.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L19_bloom_filter.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [lru_cache.h](../../src/cache/lru_cache/lru_cache.h#L7) / [.cpp](../../src/cache/lru_cache/20_lru_cache.cpp#L5) | `LRUCache` (L7), `Get` (L5), `Put` (L14) |
> | Python | [L20_lru_cache.py](../../python/algorithms/data_structures/L20_lru_cache.py#L15) | `LRUCache` (L15) |
...
**Get 操作流程**（对应 `Get` 函数，20_lru_cache.cpp#L5-12）：

```mermaid
flowchart TD
    A["Get: 开始查询"] --> B["map_.find(key)<br/><!-- src: 20_lru_cache.cpp#L7 -->"]
    B --> C{"key存在?"}
    C -->|否| D["返回 -1<br/><!-- src: 20_lru_cache.cpp#L9 -->"]
    C -->|是| E["list_.splice(begin, it)<br/>移到链表头部<br/><!-- src: 20_lru_cache.cpp#L10 -->"]
    E --> F["返回 it->second->second<br/><!-- src: 20_lru_cache.cpp#L11 -->"]
```

**Put 操作流程**（对应 `Put` 函数，20_lru_cache.cpp#L14-29）：

```mermaid
flowchart TD
    A["Put: 开始插入"] --> B["map_.find(key)<br/><!-- src: 20_lru_cache.cpp#L16 -->"]
    B --> C{"key已存在?"}
    C -->|是| D["更新value<br/>splice移到头部<br/><!-- src: 20_lru_cache.cpp#L18-19 -->"]
    C -->|否| E{"size >= capacity?"}
    E -->|是| F["淘汰尾部节点<br/>map_.erase(last.first)<br/>list_.pop_back()<br/><!-- src: 20_lru_cache.cpp#L22-25 -->"]
    E -->|否| G["list_.emplace_front(key,value)<br/>map_[key] = list_.begin()<br/><!-- src: 20_lru_cache.cpp#L27-28 -->"]
    F --> G
```
...
👉 [Get 与 Put 操作](../../src/cache/lru_cache/20_lru_cache.cpp)
...
👉 [Python 实现](../../python/algorithms/data_structures/L20_lru_cache.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [adjacency_matrix.h](../../src/graph/adjacency_matrix/adjacency_matrix.h#L13) / [.c](../../src/graph/adjacency_matrix/21_adjacency_matrix.c#L5) | `MGraph` (L13), `DFS` (L5), `Prim` (L83), `Dijkstra` (L123) |
> | Python | [L21_adjacency_matrix.py](../../python/algorithms/data_structures/L21_adjacency_matrix.py#L16) | `AdjacencyMatrix` (L16) |
...
👉 [DFS 遍历](../../src/graph/adjacency_matrix/21_adjacency_matrix.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L21_adjacency_matrix.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [adjacency_list.h](../../src/graph/adjacency_list/adjacency_list.h#L12) / [.c](../../src/graph/adjacency_list/22_adjacency_list.c#L5) | `ArcNode` (L12), `ALGraph` (L23), `Kruskal` (L116) |
> | Python | [L22_adjacency_list.py](../../python/algorithms/data_structures/L22_adjacency_list.py#L16) | `AdjacencyList` (L16) |
...
👉 [Kruskal 最小生成树](../../src/graph/adjacency_list/22_adjacency_list.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L22_adjacency_list.py)
...
> | 语言 | 文件 | 关键函数/类 |
> |------|------|------------|
> | C/C++ | [union_find.h](../../src/graph/union_find/union_find.h#L6) / [.c](../../src/graph/union_find/23_union_find.c#L30) | `UnionFind` (L6), `UFFind` (L30), `UFUnion` (L42) |
> | Python | [L23_union_find.py](../../python/algorithms/data_structures/L23_union_find.py#L16) | `UnionFind` (L16) |
...
**Find 查找 + 路径压缩流程**（对应 `UFFind` 函数，23_union_find.c#L30-40）：

```mermaid
flowchart TD
    A["UFFind: 开始查找"] --> B{"x 超出范围?<br/><!-- src: 23_union_find.c#L32 -->"}
    B -->|是| C["返回 -1"]
    B -->|否| D{"parent[x] != x?<br/><!-- src: 23_union_find.c#L36 -->"}
    D -->|否| E["返回 x<br/>x 即为根节点"]
    D -->|是| F["递归查找根<br/>parent[x] = UFFind(uf, parent[x])<br/><!-- src: 23_union_find.c#L37 -->"]
    F --> G["路径压缩: 直接连到根<br/>返回 parent[x]"]
```

**Union 合并流程**（对应 `UFUnion` 函数，23_union_find.c#L42-59）：

```mermaid
flowchart TD
    A["UFUnion: 开始合并"] --> B["查找两根<br/>rootX = UFFind(x)<br/>rootY = UFFind(y)<br/><!-- src: 23_union_find.c#L44-45 -->"]
    B --> C{"rootX == rootY?<br/><!-- src: 23_union_find.c#L48 -->"}
    C -->|是| D["已在同一集合,返回"]
    C -->|否| E{"rank[rootX] &lt; rank[rootY]?<br/><!-- src: 23_union_find.c#L50 -->"}
    E -->|是| F["parent[rootX] = rootY<br/><!-- src: 23_union_find.c#L51 -->"]
    E -->|否| G{"rank[rootX] &gt; rank[rootY]?<br/><!-- src: 23_union_find.c#L52 -->"}
    G -->|是| H["parent[rootY] = rootX<br/><!-- src: 23_union_find.c#L53 -->"]
    G -->|否| I["parent[rootY] = rootX<br/>rank[rootX]++<br/><!-- src: 23_union_find.c#L55-56 -->"]
    F --> J["count--<br/><!-- src: 23_union_find.c#L58 -->"]
    H --> J
    I --> J
```
...
👉 [查找与合并操作](../../src/graph/union_find/23_union_find.c)
...
👉 [Python 实现](../../python/algorithms/data_structures/L23_union_find.py)
